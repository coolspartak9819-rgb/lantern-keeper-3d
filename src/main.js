...
function lightLantern(item) { item.lit = true; item.light.intensity = 5; item.bulb.material.color.set(0xffc267); item.ring.material.color.set(0xffb24f); item.ring.material.opacity = .95; player.fireflyCount -= 3; player.lit++; player.score += 100 + player.lit * 25; sound(392, .25, 'sine', .04); sound(523, .4, 'sine', .04, .1); sound(659, .5, 'sine', .03, .2); showToast(`Фонарь зажжён  +${100 + player.lit * 25}`); updateHUD(); if (player.lit === lanterns.length) finish(); }
