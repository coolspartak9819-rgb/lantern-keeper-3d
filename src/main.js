...
function updateHUD() { $('score').textContent = String(player.score).padStart(4, '0'); $('fireflies').textContent = player.fireflyCount; $('lantern-count').textContent = `${player.lit} / 8 lanterns lit`; $('progress-bar').style.width = `${player.lit / 8 * 100}%`; const seconds = Math.max(0, Math.ceil(player.time)); $('time').textContent = `${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}`; }
function lightLantern(item) { item.lit = true; item.light.intensity = 5; item.bulb.material.color.set(0xffc267); item.ring.material.color.set(0xffb24f); item.ring.material.opacity = .95; player.fireflyCount -= 3; player.lit++; player.score += 100 + player.lit * 25; showToast(`Lantern lit  +${100 + player.lit * 25}`); updateHUD(); if (player.lit === lanterns.length) finish(); }
function collectFireflies(dt) { for (const fly of fireflies) { if (fly.userData.collected) continue; fly.position.y = fly.userData.baseY + Math.sin(clock.elapsedTime * 1.7 + fly.userData.phase) * .18; fly.material.opacity = .75 + Math.sin(clock.elapsedTime * 4 + fly.userData.phase) * .25; if (fly.position.distanceTo(camera.position) < 1.25) { fly.userData.collected = true; fly.visible = false; player.fireflyCount++; player.score += 10; showToast('+ 1 firefly'); updateHUD(); } } }
...
function start() { player.active = true; $('start-screen').classList.add('hidden'); renderer.domElement.requestPointerLock?.(); showToast('Find the first lantern'); }
